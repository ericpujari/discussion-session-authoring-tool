...
// Word-boundary anchored on both sides so a blocked word only matches as a
// whole word — this is what keeps "assassin" and "essex" from tripping on a
// blocked "ass" (the "Scunthorpe problem"). Multi-word phrases (e.g. "self
// harm") have their internal space turned into \s+ so they still match after
// normalizeForBlocklist collapses whitespace.
//
...
//
// A function, not a one-time const, because a coach can add a word to the
// runtime-editable list (see lib/icon-blocklist-store.js) — callers that
// have fetched that list pass it in as extraTerms to get a pattern that
// includes it; that pattern is built once per search request (not once per
// icon in a result set), then reused across isBlockedQuery/isBlockedIconResult.
function buildBlocklistPattern(extraTerms) {
  const all = BLOCKED_TERMS.concat(Array.isArray(extraTerms) ? extraTerms : []);
  const pieces = all
    .map((term) => normalizeForBlocklist(term))
    .filter(Boolean)
    .map((term) => term.split(' ').map(escapeRegExp).join('\\s+'));
  return new RegExp('\\b(' + pieces.join('|') + ')\\b', 'i');
}

// The static-only pattern (no coach-added terms) — used whenever a caller
// doesn't have a dynamic list to hand, or as the fail-open fallback if
// fetching the dynamic list from Redis fails, so a Redis hiccup degrades to
// "the built-in list still applies" rather than "nothing is blocked."
const STATIC_BLOCKLIST_PATTERN = buildBlocklistPattern([]);
...
function isBlockedQuery(rawQuery, pattern) {
  return (pattern || STATIC_BLOCKLIST_PATTERN).test(normalizeForBlocklist(rawQuery));
}
...
// of defense.
function isBlockedIconResult(rawIcon, pattern) {
  ...
  return (pattern || STATIC_BLOCKLIST_PATTERN).test(normalizeForBlocklist(pieces.join(' ')));
}
